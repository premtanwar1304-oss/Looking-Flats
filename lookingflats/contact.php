<?php
include 'db.php';

$message_sent = false;
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $conn->real_escape_string($_POST['name']);
    $phone = $conn->real_escape_string($_POST['phone']);
    $city = $conn->real_escape_string($_POST['city']);
    $message = $conn->real_escape_string($_POST['message']);

    $sql = "INSERT INTO enquiries (name, phone, city, message) VALUES ('$name', '$phone', '$city', '$message')";
    
    if ($conn->query($sql) === TRUE) {
        $message_sent = true;

        // Send Email Notification
        $to = $dealer_email;
        $subject = "New Property Enquiry - $site_name";
        $email_body = "Bhai, ek nayi lead aayi hai!\n\n";
        $email_body .= "Name: $name\n";
        $email_body .= "Phone: $phone\n";
        $email_body .= "City: $city\n";
        $email_body .= "Message: $message\n\n";
        $email_body .= "Check Admin Dashboard for more details: http://localhost/property/loookinflats/admin.php";
        
        $headers = "From: no-reply@lookingflats.com\r\n";
        $headers .= "Reply-To: no-reply@lookingflats.com\r\n";
        $headers .= "X-Mailer: PHP/" . phpversion();

        // Note: mail() function might not work on localhost without XAMPP configuration
        @mail($to, $subject, $email_body, $headers);
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contact – Lookingflats</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <header class="site-header">
        <div class="container header-inner">
            <div class="logo">
                <span class="logo-mark">L</span>
                <span class="logo-text">Lookingflats</span>
            </div>
            <nav class="main-nav">
                <a href="index.php">Home</a>
                <a href="properties.php">Properties</a>
                <a href="contact.php" class="active">Contact</a>
            </nav>
            <a href="contact.php" class="header-cta">Talk to Dealer</a>
        </div>
    </header>
    <main>
        <section class="page-header">
            <div class="container">
                <h1>Talk to Lookingflats</h1>
                <p>Have a specific property in mind? Or want to list your projects? Fill the form below.</p>
            </div>
        </section>
        <section class="section">
            <div class="container contact-layout">
                <div class="contact-details">
                    <h2>Dealer Details</h2>
                    <div class="contact-row">
                        <span>Phone:</span>
                        <span>+91 88104 33515</span>
                    </div>
                    <div class="contact-row">
                        <span>WhatsApp:</span>
                        <span>+91 88104 33515</span>
                    </div>
                    <div class="contact-row">
                        <span>Email:</span>
                        <span>pradeep@lookingflats.com</span>
                    </div>
                    <div class="contact-row">
                        <span>Office:</span>
                        <span>Sector 62, Noida</span>
                    </div>
                </div>
                <div class="contact-form-box">
                    <h2>Quick Enquiry</h2>
                    <?php if ($message_sent): ?>
                        <div style="background: #22c55e; color: #fff; padding: 15px; border-radius: 10px; margin-bottom: 15px;">
                            <strong>Success!</strong> Your enquiry has been submitted. Pradeep will contact you soon.
                        </div>
                    <?php endif; ?>
                    <form class="contact-form" method="POST">
                        <div class="form-row">
                            <label>Full Name</label>
                            <input type="text" name="name" placeholder="Enter your name" required>
                        </div>
                        <div class="form-row">
                            <label>Mobile Number</label>
                            <input type="text" name="phone" placeholder="Enter mobile number" required>
                        </div>
                        <div class="form-row">
                            <label>City Interested</label>
                            <input type="text" name="city" placeholder="e.g. Mumbai">
                        </div>
                        <div class="form-row">
                            <label>Your Message</label>
                            <textarea rows="4" name="message" placeholder="Tell us what you are looking for..."></textarea>
                        </div>
                        <button type="submit" class="btn-primary full-width">Submit Enquiry</button>
                    </form>
                </div>
            </div>
        </section>
    </main>
    <footer class="site-footer">
        <div class="container footer-inner">
            <div class="footer-brand">
                <span class="logo-mark small">L</span>
                <span class="logo-text">Lookingflats</span>
            </div>
            <p>Direct contact page for dealers. Connect your CRM or Email API here.</p>
            <p class="footer-copy">© <?php echo date('Y'); ?> Lookingflats. All rights reserved.</p>
        </div>
    </footer>
    
    <!-- WhatsApp Floating Button -->
    <a href="https://wa.me/918810433515?text=Hello%2C%20I%20want%20to%20discuss%20property%20deals%20at%20Lookingflats" class="whatsapp-float" target="_blank" title="Chat on WhatsApp">
        <svg viewBox="0 0 32 32">
            <path d="M16 3C9.373 3 4 8.373 4 15c0 2.637.86 5.08 2.48 7.13L4 29l7.13-2.48A11.93 11.93 0 0 0 16 27c6.627 0 12-5.373 12-12S22.627 3 16 3zm0 22c-1.98 0-3.91-.58-5.54-1.67l-.39-.25-4.23 1.47 1.47-4.23-.25-.39A9.94 9.94 0 0 1 6 15c0-5.514 4.486-10 10-10s10 4.486 10 10-4.486 10-10 10zm5.07-7.75c-.28-.14-1.65-.81-1.9-.9-.25-.09-.43-.14-.61.14-.18.28-.7.9-.86 1.08-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.4-.83-.74-1.39-1.65-1.55-1.93-.16-.28-.02-.43.12-.57.13-.13.28-.32.42-.48.14-.16.18-.28.28-.46.09-.18.05-.34-.02-.48-.07-.14-.84-2.01-.22-.53-.45-.46-.61-.47-.16-.01-.34-.01-.52-.01-.18 0-.48.07-.73.34-.25.27-.96.94-.96 2.3s.98 2.67 1.12 2.85c.14.18 1.93 2.95 4.68 4.02.65.28 1.16.45 1.56.58.65.21 1.24.18 1.7.11.52-.08 1.65-.67 1.88-1.32.23-.65.23-1.2.16-1.32-.07-.12-.25-.18-.53-.32z"/>
        </svg>
    </a>
</body>
</html>
